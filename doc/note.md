...
在Redis只运行**单线程的情况下，该机制允许内核中，同时存在多个监听套接字和已连接套接字。**一旦有请求到达，就会交给Redis线程处理。为了在请求到达时能够通知到Redis线程，select/epoll提供了基于事件的回调，即针对不同的事件的发生，调用相应的处理函数。
